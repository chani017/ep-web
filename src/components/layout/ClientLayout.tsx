...
import ResizableLayout from './ResizableBar'
...
export default function ClientLayout({posts, children}: ClientLayoutProps) {
  ...
  const {filteredPosts, searchTerm, selectedYear, selectedCategory} = filterState
  ...
  }, [filteredPosts, searchTerm, selectedYear, selectedCategory, setVisibleClients])

  const pathname = usePathname()
  const [mobileViewMode, setMobileViewMode] = React.useState<'mobileImg' | 'list'>('mobileImg')
  const mobileScrollRef = React.useRef<HTMLDivElement>(null)

  const [activePostContent, setActivePostContent] = React.useState<React.ReactNode>(null)
  ...
  if (isMobile) {
    ...
    return (
      <div className="flex h-screen flex-col overflow-hidden bg-background font-ep-sans text-system-white">
        <div className="flex-1 overflow-y-auto no-scrollbar" ref={mobileScrollRef}>
          <MobileHeader
        ...
        <div
          className={cn(
            'fixed inset-x-0 bottom-0 top-9 z-80 bg-background transition-transform duration-200 ease-in-out',
