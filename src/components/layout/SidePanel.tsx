...
const clientItemVariants = cva(
  'break-keep text-left text-size-sm md:text-size-md transition-colors cursor-pointer',
  {
    variants: {
      visible: {
        true: 'text-system-gray',
        false: 'text-system-white hover:text-system-gray',
      },
    },
    defaultVariants: {
      visible: false,
...
const ContactSection = ({language}: {language: string}) => (
  <>
    <h3 className="text-size-md md:text-size-lg leading-[1.4] font-semibold text-system-white font-ep-gothic break-keep">
      {language === 'kr' ? (
        <>
          일상의실천은 권준호, 김경철, 김어진이 운영하는 그래픽디자인 스튜디오입니다.
          <br />
          일상의실천은 오늘날 우리가 살아가는 현실에서 디자인이 어떤 역할을 해야 하며, 또한 무엇을
          할 수 있는가를 고민하는 소규모 공동체입니다. 그래픽디자인을 기반으로 하고 있지만, 평면
          작업에만 머무르지 않는 다양한 디자인의 방법론을 탐구하고 있습니다.
        </>
      ) : (
        <>
          Everyday Practice is a graphic design studio founded by Joonho Kwon, Kyung-chul Kim, and
          Eojin Kim.
          <br />
          Everyday Practice is a small community that contemplates what role design should play in
          the reality we live in today and what it can do. While based on graphic design, we explore
          various design methodologies that go beyond two-dimensional work.
        </>
      )}
    </h3>
...
const CvTypeSection = ({type, cvs}: {type: string | null; cvs: SanityDocument[]}) => (
...
const CvCategorySection = ({category, allCvs}: {category: string; allCvs: SanityDocument[]}) => {
  const categoryCvs = allCvs.filter((cv: SanityDocument) => cv.category === category)
  const typeOrder = ['Solo Exhibition', 'Selected Group Exhibition']
  const types = Array.from(new Set(categoryCvs.map((cv: SanityDocument) => cv.type))).sort(
    (a, b) => {
    ...
    <div className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] md:grid-cols-[repeat(auto-fill,minmax(180px,1fr))] w-full items-start justify-items-start ">
      {clients.map((client: string) => {
        ...
        return (
          <button
            key={client}
            className={cn(clientItemVariants({visible: isVisible}))}
            onClick={() => {
...
export default function SideBar({exhibition, award, clients}: SideBarProps) {
  const {activeTab, language, setSearchTerm, setIsMobileSidebarOpen, isMobile, visibleClients} =
    useAppContext()
        ...
        return (
          <div className="text-size-sm md:text-size-md font-medium text-system-white font-ep-sans space-y-10">
            {finalCategories.map((category) => (
              <CvCategorySection key={category} category={category} allCvs={allCvs} />
            ))}
          </div>
        )
      }
  ...
  return (
    <div className="p-2 flex flex-col h-full overflow-y-auto no-scrollbar break-keep">
      {renderAsideContent()}
    </div>
  )
}
