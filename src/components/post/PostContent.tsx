...
export default function PostContent({post, isMobile = false}: PostContentProps) {
  const {
    language,
    isFullContentMode,
          ...
          {Array.isArray(post.additional_link) ? (
            post.additional_link.map((link: string, index: number) => (
              <a
                ...
                className={cn(
                  'flex items-center hover:brightness-50 transition-all text-size-md font-ep-sans text-system-white',
          ...
          <div className="px-2 pb-2 flex justify-between items-start">
            ...
            <div className="flex items-start gap-x-2 mt-2">
              ...
              <div className="flex flex-col gap-0.5 items-end">
                <CategoryTag categories={post.category} className="flex-col gap-0.5 items-end" />
              </div>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col h-full relative overflow-hidden">
      {!isFullContentMode && (
        <div
          className={cn(
            'p-2 flex flex-col h-full overflow-y-auto no-scrollbar gap-4 transition-all duration-300',
      ...
      {!isFullContentMode && (
        <div
          className={cn(
            'absolute bottom-0 left-0 w-full h-12 bg-linear-to-t from-system-dark to-transparent z-50 pointer-events-none transition-opacity duration-500',
      ...
      <div
        className={cn(
          'absolute bottom-0 left-0 w-full transition-all duration-500 ease-in-out bg-system-dark z-40 flex flex-col',
      ...
      >
        <div
          ...
          className={cn(
            'flex justify-between items-start px-2 pt-2 h-full relative overflow-hidden break-keep wrap-break-word hover:bg-system-dark/80',
